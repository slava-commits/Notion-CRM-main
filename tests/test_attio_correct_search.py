#!/usr/bin/env python3
"""
Test Attio Integration - Correct Search Endpoint
Use the correct Attio API: POST /v2/records/search
"""
...
class AttioIntegrationCorrectSearch:
    """Attio CRM API integration - Correct search endpoint version"""
    ...
    async def search_people(self, query: str = "", limit: int = 10) -> List[Dict]:
        """Search for people using the correct /records/search endpoint"""
        try:
            ...
            logger.info(f"Searching for people with query: '{query}' using /records/search")
            response = await self._make_request("POST", "records/search", request_data)
    ...
    async def search_person_by_email(self, email: str) -> Optional[Dict]:
        """Search for a person by email using the correct search endpoint"""
        try:
            people = await self.search_people(query=email, limit=10)
    ...
    async def get_all_people(self) -> List[Dict]:
        """Get all people using search with empty query"""
        try:
            return await self.search_people(query="", limit=100)
        except Exception as e:
...
async def test_attio_correct_search():
    """Test Attio integration with correct search endpoint"""
    ...
    try:
        logger.info("🚀 Testing Attio Integration (Correct Search Endpoint)")
        logger.info("=" * 60)
        
        async with AttioIntegrationCorrectSearch(attio_api_key) as attio:
            ...
            async with NotionClient2025(notion_token) as notion:
                ...
                try:
                    ...
                    logger.info("• Real Attio API integration with correct /records/search endpoint")
                    logger.info("• Proper search functionality")
...
def main():
    """Main function"""
    asyncio.run(test_attio_correct_search())
