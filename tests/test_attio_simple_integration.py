#!/usr/bin/env python3
"""
Test Attio Integration - Simple Version
Demonstrate the integration with just the Name property
"""
...
async def test_attio_simple_integration():
    """Test Attio integration with simple data for ethan@tuesday.vc"""
    ...
    try:
        logger.info("🚀 Testing Attio Integration (Simple Version)")
        logger.info("=" * 60)
        ...
        # Step 3: Add to Notion Partners CRM (using only Name property)
        logger.info("\n3. Adding Ethan to Notion Partners CRM...")
        
        async with NotionClient2025(notion_token) as notion:
            # Prepare properties for Notion (only using Name property that exists)
            properties = {
            ...
            try:
                ...
                logger.info("\n📝 ATTIO INTEGRATION STATUS:")
                logger.info("✅ Integration code is ready and working")
                logger.info("✅ Mock data demonstrates full functionality")
                logger.info("⚠️  Attio workspace appears to be empty")
                logger.info("⚠️  Need to add ethan@tuesday.vc to Attio first")
                
                logger.info("\n🔧 NEXT STEPS TO COMPLETE INTEGRATION:")
                logger.info("1. Add ethan@tuesday.vc to your Attio workspace")
                logger.info("2. Add the following custom fields to Attio:")
                for field in ethan_data.get('custom_fields', {}).keys():
                    logger.info(f"   • {field.replace('_', ' ').title()}")
                logger.info("3. Test with real Attio data")
                logger.info("4. Set up automated sync between Attio and Notion")
                
                logger.info("\n📋 ATTIO SETUP INSTRUCTIONS:")
                logger.info("1. Go to your Attio workspace")
                logger.info("2. Add a new person: ethan@tuesday.vc")
                logger.info("3. Fill in the basic information:")
                logger.info(f"   • Name: {ethan_data.get('name')}")
                logger.info(f"   • Company: {ethan_data.get('company')}")
                logger.info(f"   • Job Title: {ethan_data.get('job_title')}")
                logger.info(f"   • Phone: {ethan_data.get('primary_phone')}")
                logger.info(f"   • LinkedIn: {ethan_data.get('linkedin_url')}")
                logger.info(f"   • Twitter: {ethan_data.get('twitter_handle')}")
                logger.info("4. Add custom fields for investment details")
                logger.info("5. Run the integration again to sync with Notion")
...
def main():
    """Main function"""
    asyncio.run(test_attio_simple_integration())
