#!/usr/bin/env python3
"""
Complete Attio to Notion Integration
Add all possible Attio attributes to Partners CRM database
"""
...
def create_notion_properties_from_attio(attio_data: Dict) -> Dict[str, Any]:
    """Convert Attio data to Notion properties format"""
    properties = {}
    ...
    if 'email_addresses' in values:
        ...
        if email_data:
            properties['Email'] = {"rich_text": [{"type": "text", "text": {"content": email_data}}]}
    ...
    if 'strongest_connection_strength' in values:
        ...
        if strength_select:
            properties['Relationship Quality'] = {"rich_text": [{"type": "text", "text": {"content": strength_select}}]}
...
async def add_attio_attributes_to_notion():
    """Add all Attio attributes to Notion Partners CRM database"""
    ...
    try:
        logger.info("🚀 Adding All Attio Attributes to Notion Partners CRM")
        logger.info("=" * 60)
        ...
        # Step 2: Check current Notion database schema
        async with NotionClient2025(notion_token) as notion:
            logger.info("\n2. Checking current Notion database schema...")
            ...
            # Step 3: Convert Attio data to Notion properties
            logger.info("\n3. Converting Attio data to Notion properties...")
            
            notion_properties = create_notion_properties_from_attio(attio_data)
            
            logger.info(f"Converted {len(notion_properties)} properties:")
            for prop_name in notion_properties.keys():
                logger.info(f"  • {prop_name}")
            
            # Step 4: Create page with all properties
            logger.info("\n4. Creating Notion page with all Attio attributes...")
            ...
            try:
                ...
                logger.info(f"• Notion Database: ✅ Partners CRM")
                ...
                # Try with just basic properties if full creation fails
                logger.info("\n🔄 Trying with basic properties only...")
                basic_properties = {
                    "Name": notion_properties.get("Name", {"title": [{"type": "text", "text": {"content": "Ethan Imboden"}}]})
                }
                
                try:
                    ethan_page = await notion.create_page_in_database(partners_db_id, basic_properties)
                    logger.info(f"✅ Basic page created: {ethan_page['url']}")
                    logger.info("Note: Some properties may not exist in the database schema")
                except Exception as e2:
                    logger.error(f"❌ Failed to create even basic page: {e2}")
                    return
...
def main():
    """Main function"""
    asyncio.run(add_attio_attributes_to_notion())
