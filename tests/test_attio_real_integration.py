#!/usr/bin/env python3
"""
Test Attio Integration - Real Data Version
Retrieve ethan@tuesday.vc from Attio using the correct API endpoint
"""
...
class AttioIntegrationReal:
    ...
    async def search_person_by_email(self, email: str) -> Optional[Dict]:
        """Search for a person by email using the correct Attio API endpoint"""
        try:
            # Use the correct Attio API endpoint for filtering people records
            search_data = {
                "filter": {
                    "email_addresses": {
                        "email_address": {
            ...
            response = await self._make_request("POST", "objects/people/records", search_data)
            
            data = response.get('data', [])
            if data and len(data) > 0:
                return await self._parse_person(data[0])
...
async def test_attio_real_integration():
    """Test Attio integration with real data for ethan@tuesday.vc"""
    ...
    try:
        ...
        # Step 1: Search for Ethan in Attio
        logger.info("\n1. Searching for ethan@tuesday.vc in Attio...")
        
        async with AttioIntegrationReal(attio_api_key) as attio:
            ethan_data = await attio.search_person_by_email("ethan@tuesday.vc")
            
            if not ethan_data:
                logger.error("❌ ethan@tuesday.vc not found in Attio")
                logger.info("Let me try alternative search methods...")
                
                # Try different search approaches
                alternative_searches = [
                    "ethan@tuesday.vc",
                    "ethan.imboden@tuesday.vc",
                    "ethan",
                    "tuesday.vc"
                ]
                
                for search_term in alternative_searches:
                    logger.info(f"Trying search for: {search_term}")
                    try:
                        ...
                        response = await attio._make_request("POST", "objects/people/records", search_data)
                        data = response.get('data', [])
                        
                        if data and len(data) > 0:
                            logger.info(f"✅ Found {len(data)} records for '{search_term}'")
                            ethan_data = await attio._parse_person(data[0])
                            break
                        else:
                ...
                if not ethan_data:
                    logger.error("❌ Still no data found. Let me try to list all people...")
                    ...
                    try:
                        ...
                        response = await attio._make_request("POST", "objects/people/records", all_people_data)
                        data = response.get('data', [])
                        
                        logger.info(f"Found {len(data)} total people in Attio")
                        
                        if data:
                            logger.info("First few people:")
                            for i, person in enumerate(data[:3]):
                                name = person.get('attributes', {}).get('name', {}).get('value', 'Unknown')
                                emails = person.get('attributes', {}).get('email_addresses', {}).get('value', [])
                                logger.info(f"  {i+1}. {name} - {emails}")
                                
                                # Check if this might be Ethan
                                if 'ethan' in name.lower() or any('ethan' in str(email).lower() for email in emails):
                                    logger.info(f"  ✅ This might be Ethan: {name}")
                                    ethan_data = await attio._parse_person(person)
                                    break
                ...
                if not ethan_data:
                    logger.error("❌ Could not find ethan@tuesday.vc in Attio")
                    return
            else:
            ...
            async with NotionClient2025(notion_token) as notion:
                ...
                try:
                    ...
                    logger.info("• Real Attio API integration")
                    logger.info("• Correct API endpoint usage")
