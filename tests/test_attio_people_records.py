#!/usr/bin/env python3
"""
Test Attio Integration - Access People Records Directly
Try to access the People records that exist in the Attio interface
"""
...
class AttioPeopleRecords:
    """Attio CRM API integration - Access People records directly"""
    
    def __init__(self, api_key: str):
        self.api_key = api_key
        self.base_url = "https://api.attio.com/v2"
        self.session = None
        self.people_object_id = "82febeeb-fef5-42f0-a91a-6a8956783666"  # From our previous discovery
    ...
    async def get_people_records(self) -> List[Dict]:
        """Try different approaches to get People records"""
        approaches = [
            # Direct object ID approaches
            ...
            f"lists/{self.people_object_id}",
            f"lists/{self.people_object_id}/entries",
            f"lists/{self.people_object_id}/records",
            
            # Alternative patterns
            f"objects/people/records",
            f"people/records",
            f"records/people",
            
            # With filters
            f"objects/{self.people_object_id}/records?limit=10",
            f"lists/{self.people_object_id}?limit=10",
        ]
        
        for approach in approaches:
            try:
                logger.info(f"Trying: GET {approach}")
                response = await self._make_request("GET", approach)
                ...
                if 'data' in response:
                    ...
                    if isinstance(data, list) and len(data) > 0:
                        ...
                        # Check if these look like person records
                        first_record = data[0]
                        if 'attributes' in first_record or 'name' in first_record:
                            logger.info("✅ These look like person records!")
                            return data
                        else:
                            logger.info("❌ These don't look like person records")
        ...
        post_approaches = [
            ...
            (f"lists/{self.people_object_id}/entries", {"filter": {}}),
            (f"objects/people/records", {"filter": {}}),
            (f"records/people", {"filter": {}}),
        ]
    ...
    async def search_person_by_email(self, email: str) -> Optional[Dict]:
        """Search for a person by email in the retrieved records"""
        try:
            records = await self.get_people_records()
            
            if not records:
                logger.error("No people records found")
                return None
            
            logger.info(f"Searching through {len(records)} people records for {email}")
            
            for person in records:
                # Try different ways to extract email
                emails = []
                
                # Method 1: attributes.email_addresses
                if 'attributes' in person:
                    attrs = person['attributes']
                    if 'email_addresses' in attrs:
                        email_data = attrs['email_addresses']
                ...
                if 'email_address' in person:
                    emails.append(person['email_address'])
                
                # Check for email match
                for person_email in emails:
                    if email.lower() in person_email.lower():
                        logger.info(f"✅ Found person with email {person_email}")
                        return await self._parse_person(person)
            
            logger.info(f"❌ No person found with email {email}")
            logger.info("Available people:")
            for i, person in enumerate(records[:5]):  # Show first 5
                name = person.get('attributes', {}).get('name', {}).get('value', 'Unknown')
                emails = person.get('attributes', {}).get('email_addresses', {})
                if isinstance(emails, dict):
                    emails = emails.get('value', [])
                logger.info(f"  {i+1}. {name} - {emails}")
...
async def test_attio_people_records():
    """Test Attio integration by accessing People records directly"""
    ...
    try:
        logger.info("🚀 Testing Attio Integration (People Records Direct Access)")
        logger.info("=" * 60)
        
        async with AttioPeopleRecords(attio_api_key) as attio:
            # Step 1: Get People records
            logger.info("\n1. Getting People records from Attio...")
            
            people_records = await attio.get_people_records()
            
            if not people_records:
                logger.error("❌ No People records found in Attio")
                logger.info("This could mean:")
                logger.info("1. The People section is empty")
                logger.info("2. The API endpoints are different than expected")
                logger.info("3. The API key doesn't have the required permissions")
                return
            
            logger.info(f"✅ Found {len(people_records)} People records")
            
            # Step 2: Search for Ethan
            logger.info("\n2. Searching for ethan@tuesday.vc...")
            
            ethan_data = await attio.search_person_by_email("ethan@tuesday.vc")
            
            if not ethan_data:
                logger.error("❌ ethan@tuesday.vc not found in People records")
                return
            else:
            ...
            async with NotionClient2025(notion_token) as notion:
                ...
                try:
                    ...
                    logger.info("• Real Attio API integration with People records")
                    logger.info("• Direct access to People section")
...
def main():
    """Main function"""
    asyncio.run(test_attio_people_records())
