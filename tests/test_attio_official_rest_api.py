#!/usr/bin/env python3
"""
Test Attio Integration - Official REST API Documentation
Use the correct endpoints from the official REST API docs
"""
...
class AttioIntegrationOfficialREST:
    """Attio CRM API integration - Official REST API version"""
    ...
    async def explore_api_structure(self):
        """Explore the Attio API structure to find working endpoints"""
        logger.info("🔍 Exploring Attio API structure...")
        
        # Test core endpoints from the official docs
        endpoints_to_test = [
            # Core endpoints
            ("GET", "objects"),
            ("GET", "objects/people"),
            ...
            # Standard objects - try different patterns
            ("GET", "people"),
            ("GET", "people/records"),
            ("POST", "people/records"),
            ("GET", "records/people"),
            ...
            try:
                ...
                if method == "GET":
                    ...
                    if 'data' in response and isinstance(response['data'], list):
                        ...
                        working_endpoints.append((method, endpoint, None))
                    else:
                    ...
                    for i, post_data in enumerate(post_data_options):
                        try:
                            ...
                            if 'data' in response and isinstance(response['data'], list):
                                logger.info(f"  ✅ SUCCESS with POST data option {i+1}")
                                working_endpoints.append((method, endpoint, post_data))
                                success = True
                                break
                        except:
                            continue
    ...
    async def search_person_by_email(self, email: str) -> Optional[Dict]:
        """Search for a person by email using discovered endpoints"""
        try:
            # First, explore the API to find working endpoints
            working_endpoints = await self.explore_api_structure()
            ...
            # Try to find a person using the working endpoints
            for method, endpoint, post_data in working_endpoints:
                try:
                    if method == "GET":
                        response = await self._make_request("GET", endpoint)
                    else:
                    ...
                    if 'data' in response and isinstance(response['data'], list):
                        for person in response['data']:
                            if person.get('object') == 'people' or 'people' in endpoint:
                                ...
                                if email.lower() in [e.lower() for e in emails]:
                                    logger.info(f"✅ Found person with email {email}")
                                    return await self._parse_person(person)
                
                except Exception as e:
                    logger.warning(f"Failed to search with {method} {endpoint}: {e}")
                    continue
...
async def test_attio_official_rest_api():
    """Test Attio integration using official REST API documentation"""
    ...
    try:
        logger.info("🚀 Testing Attio Integration (Official REST API)")
        logger.info("=" * 60)
        
        async with AttioIntegrationOfficialREST(attio_api_key) as attio:
            # Step 1: Search for Ethan using discovered endpoints
            logger.info("\n1. Searching for ethan@tuesday.vc...")
            ...
            if not ethan_data:
                ...
                logger.info("1. The person doesn't exist in your Attio workspace")
            ...
            async with NotionClient2025(notion_token) as notion:
                ...
                try:
                    ...
                    logger.info("• Real Attio API integration using official REST API")
                    logger.info("• Dynamic endpoint discovery")
...
def main():
    """Main function"""
    asyncio.run(test_attio_official_rest_api())
