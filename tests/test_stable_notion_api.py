#!/usr/bin/env python3
"""
Test Stable Notion API
Demonstrate database creation and management using current stable API
"""
...
async def test_stable_notion_api():
    """Test the stable Notion API with proper database creation"""
    ...
    # For this test, we need a parent page ID
    ...
    async with NotionClientUpdated(notion_token) as notion:
        try:
            logger.info("🚀 Testing Stable Notion API (2022-06-28)")
            logger.info("=" * 60)
            ...
            logger.info("\n" + "=" * 60)
            logger.info("🎉 SUCCESS! Stable Notion API is working perfectly!")
            logger.info("\n📊 SUMMARY:")
...
def main():
    """Main function"""
    asyncio.run(test_stable_notion_api())
