#!/usr/bin/env python3
"""
Test Attio Integration - GET Method Version
Use the correct GET method with query parameters for Attio API
"""
...
class AttioIntegrationGET:
    """Attio CRM API integration - GET method version"""
    ...
    async def search_person_by_email(self, email: str) -> Optional[Dict]:
        """Search for a person by email using GET method with query parameters"""
        try:
            ...
            logger.info(f"Searching for {email} with params: {params}")
            response = await self._make_request("GET", "objects/people/records", params)
    ...
    async def get_all_people(self) -> List[Dict]:
        """Get all people using GET method"""
        try:
            # Use GET method with limit parameter
            params = {
                "limit": 10
            }
            
            logger.info("Getting all people with params:", params)
            response = await self._make_request("GET", "objects/people/records", params)
...
async def test_attio_get_method():
    """Test Attio integration with GET method"""
    ...
    try:
        logger.info("🚀 Testing Attio Integration (GET Method Version)")
        logger.info("=" * 60)
        
        async with AttioIntegrationGET(attio_api_key) as attio:
            ...
            async with NotionClient2025(notion_token) as notion:
                ...
                try:
                    ...
                    logger.info("• Real Attio API integration with GET method")
                    logger.info("• Correct query parameter format")
...
def main():
    """Main function"""
    asyncio.run(test_attio_get_method())
