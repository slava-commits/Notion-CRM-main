#!/usr/bin/env python3
"""
Test Attio Integration - Correct Endpoint Version
Use the correct Attio API endpoint: objects/people
"""
...
class AttioIntegrationCorrect:
    """Attio CRM API integration - Correct endpoint version"""
    ...
    async def get_people(self, limit: int = 100) -> List[Dict]:
        """Get all people from Attio using correct endpoint"""
        try:
            logger.info(f"Getting people from Attio with limit: {limit}")
    ...
    async def search_person_by_email(self, email: str) -> Optional[Dict]:
        """Search for a person by email using the search endpoint"""
        try:
            ...
            logger.info(f"Searching for {email} using search endpoint")
            response = await self._make_request("POST", "search", search_data)
            
            results = response.get('data', [])
            if results:
                logger.info(f"Found {len(results)} results")
                return await self._parse_person(results[0])
...
async def test_attio_correct_endpoint():
    """Test Attio integration with correct endpoint"""
    ...
    try:
        logger.info("🚀 Testing Attio Integration (Correct Endpoint Version)")
        logger.info("=" * 60)
        ...
        async with AttioIntegrationCorrect(attio_api_key) as attio:
            ...
            all_people = await attio.get_people(limit=10)
            ...
            # Step 2: Search for Ethan specifically using search endpoint
            logger.info("\n2. Searching for ethan@tuesday.vc using search endpoint...")
            
            ethan_data = await attio.search_person_by_email("ethan@tuesday.vc")
            
            if not ethan_data:
                logger.info("Ethan not found with search. Checking all people for Ethan...")
            ...
            async with NotionClient2025(notion_token) as notion:
                ...
                try:
                    ...
                    logger.info("• Real Attio API integration with correct endpoint")
...
def main():
    """Main function"""
    asyncio.run(test_attio_correct_endpoint())
