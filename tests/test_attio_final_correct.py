#!/usr/bin/env python3
"""
Test Attio Integration - Final Correct Version
Use the correct Attio API: POST /v2/objects/people/records with filter
"""
...
class AttioIntegrationFinal:
    """Attio CRM API integration - Final correct version"""
    ...
    async def get_all_people(self) -> List[Dict]:
        """Get all people from Attio using correct POST method"""
        try:
            # Use POST method with empty filter to get all people
            request_data = {
                "filter": {}
            }
            
            logger.info("Getting all people from Attio using POST method")
            ...
            logger.error(f"Failed to get all people from Attio: {e}")
            return []
    
    async def search_person_by_email(self, email: str) -> Optional[Dict]:
        """Search for a person by email using correct POST method with filter"""
        try:
            # Use POST method with email filter
            request_data = {
                "filter": {
                    "email_addresses": {
                        "email_address": {
            ...
            logger.info(f"Searching for {email} using POST method with filter")
            response = await self._make_request("POST", "objects/people/records", request_data)
...
async def test_attio_final_correct():
    """Test Attio integration with final correct method"""
    ...
    try:
        logger.info("🚀 Testing Attio Integration (Final Correct Version)")
        logger.info("=" * 60)
        ...
        async with AttioIntegrationFinal(attio_api_key) as attio:
            ...
            async with NotionClient2025(notion_token) as notion:
                ...
                try:
                    ...
                    logger.info("• Real Attio API integration with correct POST method")
                    logger.info("• Proper filter usage")
...
def main():
    """Main function"""
    asyncio.run(test_attio_final_correct())
