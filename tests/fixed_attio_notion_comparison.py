#!/usr/bin/env python3
"""
Fixed comparison between Attio data structure and Notion data structure
"""
...
def extract_attio_value(attr_data, key='value'):
    """Extract value from Attio attribute with better handling"""
    ...
    if isinstance(item, dict):
        ...
        elif 'interacted_at' in item:
            return item['interacted_at']
        elif 'title' in item:
            return item['title']
        elif 'target_record_id' in item:
            return item['target_record_id']
        elif 'referenced_actor_id' in item:
...
def extract_notion_value(prop_data, prop_type):
    """Extract value from Notion property based on type with better error handling"""
    if not prop_data:
        return None
    ...
    try:
        ...
        logger.warning(f"Error extracting Notion value: {e}")
...
async def compare_attio_notion_mapping():
    ...
    try:
        ...
        for attr_name, attr_info in attio_analysis.items():
            status = "✅" if attr_info['has_data'] else "❌"
            logger.info(f"  {status} {attr_name} ({attr_info['type']}): {attr_info['value']}")
        ...
        for attio_field, notion_field in expected_mappings.items():
            ...
            attio_has_data = attio_info.get('has_data', False)
