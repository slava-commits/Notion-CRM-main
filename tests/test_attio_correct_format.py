#!/usr/bin/env python3
"""
Test Attio Integration - Correct Format Version
Use the correct request format for Attio API
"""
...
class AttioIntegrationCorrect:
    """Attio CRM API integration - Correct format version"""
    ...
    async def search_person_by_email(self, email: str) -> Optional[Dict]:
        """Search for a person by email using the correct Attio API format"""
        try:
            # Use the correct Attio API format with data wrapper
            search_data = {
                "data": {
                    "filter": {
                        "email_addresses": {
            ...
            logger.info(f"Searching for {email} with data: {search_data}")
    ...
    async def get_all_people(self) -> List[Dict]:
        """Get all people using the correct format"""
        try:
            # Use the correct Attio API format
            request_data = {
                "data": {
                    "limit": 10
                }
            ...
            logger.info("Getting all people with data:", request_data)
            response = await self._make_request("POST", "objects/people/records", request_data)
...
async def test_attio_correct_format():
    """Test Attio integration with correct API format"""
    ...
    try:
        logger.info("🚀 Testing Attio Integration (Correct Format Version)")
        logger.info("=" * 60)
        
        async with AttioIntegrationCorrect(attio_api_key) as attio:
            # Step 1: Try to get all people first
            ...
            async with NotionClient2025(notion_token) as notion:
                ...
                try:
                    ...
                    logger.info("• Correct API request format")
...
def main():
    """Main function"""
    asyncio.run(test_attio_correct_format())
