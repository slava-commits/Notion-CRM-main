#!/usr/bin/env python3
"""
Test Attio Integration - Official Documentation Version
Use the correct Attio API endpoints from official docs
"""
...
class AttioIntegrationOfficial:
    """Attio CRM API integration - Official documentation version"""
    ...
    async def search_person_by_email(self, email: str) -> Optional[Dict]:
        """Search for a person by email using the official search endpoint"""
        try:
            # Use the official search endpoint from the documentation
            search_data = {
                "query": email,
                "objects": ["people"],
                "limit": 10
            ...
            logger.info(f"Searching for {email} using official search endpoint")
            response = await self._make_request("POST", "records/search", search_data)
            ...
            if data and len(data) > 0:
                ...
                for person in data:
                    if person.get('object') == 'people':
                        attributes = person.get('attributes', {})
                        emails = attributes.get('email_addresses', [])
                        
                        if email.lower() in [e.lower() for e in emails]:
                            logger.info(f"✅ Found exact email match: {person.get('record_id')}")
                            return await self.get_person_by_id(person.get('record_id'))
                
                # If no exact match, return the first person found
                logger.info("No exact email match, returning first result")
                return await self.get_person_by_id(data[0].get('record_id'))
    ...
    async def get_person_by_id(self, record_id: str) -> Optional[Dict]:
        """Get full person attributes by record ID"""
        try:
            logger.info(f"Getting full person record for ID: {record_id}")
            response = await self._make_request("GET", f"records/people/{record_id}")
            
            return await self._parse_person(response.get('data', {}))
            
        except Exception as e:
            logger.error(f"Failed to get person by ID {record_id}: {e}")
            return None
    
    async def assert_person_by_email(self, email: str, name: str = None) -> Optional[Dict]:
        """Assert (find-or-create) a person by email"""
        try:
            # Use the official assert endpoint
            assert_data = {
            ...
            logger.info(f"Asserting person for {email}")
            response = await self._make_request("POST", "objects/people/records/assert", assert_data)
            
            record_id = response.get('record_id')
            if record_id:
                logger.info(f"✅ Person asserted with ID: {record_id}")
                return await self.get_person_by_id(record_id)
            
            return None
            
        except Exception as e:
            logger.error(f"Failed to assert person by email {email}: {e}")
            return None
...
async def test_attio_official_docs():
    """Test Attio integration using official documentation"""
    ...
    try:
        logger.info("🚀 Testing Attio Integration (Official Documentation)")
        logger.info("=" * 60)
        
        async with AttioIntegrationOfficial(attio_api_key) as attio:
            # Step 1: Search for Ethan using official search endpoint
            logger.info("\n1. Searching for ethan@tuesday.vc using official search endpoint...")
            
            ethan_data = await attio.search_person_by_email("ethan@tuesday.vc")
            
            if not ethan_data:
                logger.info("Ethan not found. Trying to assert (create) the person...")
                
                # Try to assert/create the person
                ethan_data = await attio.assert_person_by_email("ethan@tuesday.vc", "Ethan Imboden")
                
                if not ethan_data:
                    logger.error("❌ Could not find or create ethan@tuesday.vc in Attio")
                    return
                else:
                    logger.info("✅ Created Ethan in Attio")
            else:
            ...
            async with NotionClient2025(notion_token) as notion:
                ...
                try:
                    ...
                    logger.info("• Real Attio API integration using official documentation")
                    logger.info("• Search records endpoint")
                    logger.info("• Assert person endpoint (find-or-create)")
                    logger.info("• Get person record endpoint")
...
def main():
    """Main function"""
    asyncio.run(test_attio_official_docs())
