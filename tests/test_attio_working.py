#!/usr/bin/env python3
"""
Test Attio Integration - Working Version
Use the correct Attio API: GET objects/people
"""
...
class AttioIntegrationWorking:
    """Attio CRM API integration - Working version using GET objects/people"""
    ...
    async def get_all_people(self) -> List[Dict]:
        """Get all people from Attio using the working GET objects/people endpoint"""
        try:
            logger.info("Getting all people from Attio using GET objects/people")
            response = await self._make_request("GET", "objects/people")
    ...
    async def search_person_by_email(self, email: str) -> Optional[Dict]:
        """Search for a person by email in the people records"""
        try:
...
async def test_attio_working():
    """Test Attio integration with working endpoint"""
    ...
    try:
        logger.info("🚀 Testing Attio Integration (Working Version)")
        logger.info("=" * 60)
        
        async with AttioIntegrationWorking(attio_api_key) as attio:
            ...
            async with NotionClient2025(notion_token) as notion:
                ...
                try:
                    ...
                    logger.info("• Real Attio API integration with correct GET objects/people endpoint")
...
def main():
    """Main function"""
    asyncio.run(test_attio_working())
